...
# suppress noisy package warnings generated by open_clip / transformers
...
from data.raw_input import ingest_report, ingest_scan, ingest_scan_with_analysis
                ...
                with cols[img_index % 3]:
                    ...
                    if st.button(f"Analyse {file.name}", key=f"scan_{file.name}"):
                        with st.spinner(f"Processing {file.name} …"):
                            ...
                            result = ingest_scan_with_analysis(
                                tmp_path,
                        ...
                        if ingestion.get("status") == "success":
                            st.success(
                                f"✓ Scan stored  |  "
                                f"Modality: **{ingestion['modality']}**  |  "
                                f"Anatomy: **{ingestion['anatomy']}**"
                            )
                        elif ingestion.get("status") == "already_ingested":
                            st.info("This scan was already ingested.")
                        else:
                            st.error("Ingestion failed – check logs.")

                        # Display disease analysis if available
                        analysis = result.get("analysis")
                        if analysis:
                            st.markdown("---")
                            st.markdown("### 🔬 Disease Analysis Report")
                            
                            # Disease findings section
                            with st.expander("📊 Image Findings & Probable Conditions", expanded=True):
                                st.markdown(analysis["disease_findings"])
                                st.metric(
                                    "Disease Confidence",
                                    f"{analysis['disease_confidence']:.0%}"
                                )
                            ...
                            if is_related:
                                # RELATED SECTION: Image and reports together
                                with st.expander("🔗 Correlation with Patient History", expanded=True):
                                    st.markdown(
                                        f"**Status:** {analysis['relationship_explanation']}"
                                    )
                                    
                                    # Show composite analysis
                                    st.markdown("**Composite Clinical Analysis:**")
                                    st.markdown(analysis["composite_analysis"])
                                    
                                    # Show related reports
                                    if analysis["related_reports"]:
                                        st.markdown("**Related Reports in History:**")
                                        for i, report in enumerate(analysis["related_reports"], 1):
                                            col1, col2 = st.columns([3, 1])
                                            with col1:
                                                st.write(
                                                    f"**{i}. {report['filename']}** "
                                                    f"({report['modality']} · {report['anatomy']})"
                                                )
                                                if report.get("text"):
                                                    st.caption(report["text"][:150])
                                            with col2:
                                                st.metric(
                                                    "Similarity",
                                                    f"{report['similarity']:.0%}"
                                                )
                            else:
                                # STANDALONE SECTION: Image analysis only
                                with st.expander("📍 Standalone Imaging Study", expanded=True):
                                    st.markdown(
                                        "**Status:** This scan represents an independent imaging study "
                                        "without directly comparable prior studies in the patient database."
                                    )
                                    st.markdown("**Clinical Impression:**")
                                    st.markdown(analysis["composite_analysis"])
                        elif result.get("analysis_error"):
                            st.warning(
                                f"Analysis unavailable: {result['analysis_error']}\n\n"
                                "Image was ingested successfully but disease analysis could not be completed."
                            )
